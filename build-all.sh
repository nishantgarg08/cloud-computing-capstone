mvn install
cd monitoring/ && mvn clean compile assembly:single && cd ..

docker build -t bhaab01/ccc:hot-partition .
docker push bhaab01/ccc:hot-partition

docker build -f Dockerfile_python -t bhaab01/ccc:python-hot-partition .
docker push bhaab01/ccc:python-hot-partition
